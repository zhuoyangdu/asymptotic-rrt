...
namespace planning {
    ...
    public:
        ...
        struct Compare2 {
            Compare2(Node sample) {this->sample = sample;}
            bool operator() (Node& a, Node& b) {
                int dist1 = (a.row() - sample.row()) * (a.row() - sample.row())  +
                (a.col() - sample.col()) * (a.col() - sample.col()) ;
                int dist2 = (b.row() - sample.row()) * (b.row() - sample.row())  +
        ...
        static int SqureDistance(const Node& p1, const Node& p2) {
            return (p1.row() - p2.row()) * (p1.row() - p2.row()) +
            (p1.col() - p2.col()) * (p1.col() - p2.col());
        }

        static int SqureDistance(const cv::Point& p1, const Node& p2) {
        ...
        vector<Node> kNearestPointsThread(const Node& sample, int k);

        static void GetCandidatesInPivot(int sample_pivot,
                                int candidate_pivot,
                                const Node& sample,
                                double threshold,
                                int maximum_size,
    ...
    void NearestInPivot(const Node& sample, int pivot_index);

    Node NearestPoint(const Node& sample);

    std::vector<std::vector<int>> DividePivots(
        const std::vector<int>& pivots, int nums);

    bool PivotIsFeasible(int candidate_pivot,
                           int sample_pivot,
                           double threshold);
